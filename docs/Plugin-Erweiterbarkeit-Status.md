...
|---|---|---|
| A — Ausgangspunkt, Umfang, Testaufbau | erledigt | `app/composer.json` ist `fkde/nafinity`; HEAD war `a194993`, nicht der Auditstand `016931e`; `AGENTS.md`, `README.md`, `docs/Implementation.md`, `docs/Nafinity-Prototypplan.md` (heute `docs/history/`), `docs/Code-Style.md` (heute in `AGENTS.md`), `REPORT.md` und die Audit-Probe gelesen; Branch angelegt; diese Datei. Die Audit-Probe läuft unverändert als Ausgangsregressionstest, Ergebnis in [`docs/Plugin-Erweiterbarkeit-Probe.json`](Plugin-Erweiterbarkeit-Probe.json). Die Abnahme läuft mit A/B, ohne A/B und mit vertauschter Plugin-Auflistung |
